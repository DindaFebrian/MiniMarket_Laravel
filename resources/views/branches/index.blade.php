...
@section('content')
    <div class="bg-gray-900 shadow rounded-lg p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-pink-400">Branches</h1>
                ...
                <tbody>
                    @forelse ($branches as $branch)
                        <tr class="hover:bg-gray-700">
                            <td class="px-4 py-2 text-pink-300 border-b border-gray-600">{{ $branch->id }}</td>
                            <td class="px-4 py-2 text-pink-300 border-b border-gray-600">{{ $branch->branch_name }}</td>
                            <td class="px-4 py-2 text-pink-300 border-b border-gray-600">{{ $branch->address }}</td>
                            <td class="px-4 py-2 text-pink-300 border-b border-gray-600">{{ $branch->city }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-2 text-center text-pink-500">
                                No branches found.
                            </td>
                        </tr>
                    @endforelse
